//gcc -o vis vishaymastercodetemp.cpp -lwiringPi
 // ./mastercode.c
 //sudo i2cdetect -y 1
 ...
 int main(int argc, char const *argv[]) {
     ...
     tempsensor = wiringPiI2CSetup(SLAVE_ADDRESS3);
     if (tempsensor == -1) {
         printf("Fout bij het openen van slave 2\n");
         return 1;
     }
             ...
             if (FD_ISSET(sd, &readfds)) {
                 ...
                 if (valread > 0) {
                     ...
                     if(strcmp(buffer, "Status") == 0) {
                         for (i = 0; i < num_clients; i++) {
                                wiringPiI2CWriteReg16(tempsensor, 0x24, 0x00);
                                delay(15); // korte wachttijd zodat sensor tijd heeft om te meten
                            
                                // Lees 6 bytes: 2 voor temp, 1 CRC, 2 voor RH, 1 CRC
                                uint8_t data[6];
                                for (int j = 0; j < 6; j++) {
                                    data[j] = wiringPiI2CRead(tempsensor);
                                }
                            
                                // Verwerk temperatuur (eerste twee bytes)
                                int temp_raw = (data[0] << 8) | data[1];
                                float temperature = -45.0 + (175.0 * ((float)temp_raw / 65535.0));
                            
                                // Stuur resultaat naar client
                                char message[64];
                                snprintf(message, sizeof(message), "Temperatuur: %.2f °C", temperature);
