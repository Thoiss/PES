 ...
 uint8_t RX_Buffer = 0;
  ...
  while (1)
  {
	  if (HAL_I2C_Slave_Receive(&hi2c1, &RX_Buffer, 1, 1000) == HAL_OK) {
	              ...
	              // Als de master een verzoek stuurt (bijvoorbeeld 0x01 voor knopstatus)
	              if (RX_Buffer == 1) {  // Stel, als de master 1 verstuurt, vraagt hij om de knopstatus
	                  // Lees de knopstatus (bijvoorbeeld of de knop is ingedrukt)
