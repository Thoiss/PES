...
const char *wemos_ips[] = {
    "192.168.137.14",  // Wemos 1
    "192.168.137.57"  // Wemos 2
};
...
int connect_to_wemos(const char *ip) { // Verbind met Wemos
    int sock;
    ...
    if (inet_pton(AF_INET, ip, &serv_addr.sin_addr) <= 0) { // ip adres toewijzen
        printf("Ongeldig adres of niet ondersteund: %s\n", ip);
        return -1;
    }

    if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) { // maak verbinding met apparaat (wemos)
...
int connect_to_device(const char *ip, int port) { // Verbind met Pi-a
    int sock;
    struct sockaddr_in serv_addr;
...
int vraag_knop_status(int Socket) { // Stuur request naar wemos voor knopstatus
...
//Toevoegen-aanpassen 5,D1  (Destination socket, Destination Pin)
    ...
    printf("%s\n", buffer);
    // usleep(500000);
    if (strcmp(buffer, "TRUE") == 0) { // kijk of wat ontvangen is vanaf Arduino IDE code == 'TRUE'
        Waarde_Knop = 1;
        Versturen = true;
    }
    memset(buffer, 0, sizeof(buffer));
...
int main() {
    ...
    for(int i = 0; i < Devices; i++){ // uitlezen
    // sleep(1);
